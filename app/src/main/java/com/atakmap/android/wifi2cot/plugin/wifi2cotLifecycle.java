...
public class wifi2cotLifecycle implements Lifecycle {
    ...
    public wifi2cotLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        wifi2cotLifecycle.this.overlays
                .add(new wifi2cotMapComponent());

        // create components
        Iterator<MapComponent> iter = wifi2cotLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(wifi2cotLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        wifi2cotLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
